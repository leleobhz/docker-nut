...
for branch in v2.8.3 v2.8.4 v2.8.5; do
	...
	buildah build --annotation "org.opencontainers.image.description=Container with NUT for UPS support" --cache-from ghcr.io/leleobhz/docker-nut --cache-to ghcr.io/leleobhz/docker-nut --jobs=$(nproc --ignore=2) --platform linux/amd64,linux/arm64 --manifest docker-nut --build-arg branch=${branch} .
